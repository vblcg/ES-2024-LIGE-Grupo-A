...
public class UserUploadFile extends JFrame implements FileCallback {
     ...
     * A interface grafica instancia-se neste local, com a criacao de botoes para o
     * efeito.
     *
     * @param fileholder Array de arquivos para armazenar o arquivo selecionado pelo
     *                   usuário.
    ...
    public UserUploadFile(File[] fileholder, String jsonFile) {
        ...
        buttonNetwork.setBounds(20, 20, 250, 50); 
        buttonNetwork.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                if (!uploaded) {
                    JOptionPane.showMessageDialog(panel, "Ainda não carregou nenhum horário!", "Erro",
                            JOptionPane.INFORMATION_MESSAGE);
                    ...
                    Desktop desk = Desktop.getDesktop();
                    try {
                        desk.browse(new URI("http://localhost/NetworkGraph.html"));
                    } catch (IOException e1) {
                        e1.printStackTrace();
                    } catch (URISyntaxException e1) {
                        e1.printStackTrace();
        ...
        buttonHeatMap.setBounds(20, 20, 250, 50); 
        buttonHeatMap.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                if (!uploaded) {
                    ...
                    Desktop desk = Desktop.getDesktop();
                    try {
                        desk.browse(new URI("http://localhost/heatmap.html"));
                    } catch (IOException e1) {
                        e1.printStackTrace();
                    } catch (URISyntaxException e1) {
                        e1.printStackTrace();
    ...
    public void checkCsvStructure(File file) throws IOException {
        ...
        String[] expected = { "Curso", "Unidade Curricular", "Turno", "Turma", "Inscritos no turno", "Dia da semana",
